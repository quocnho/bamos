...
1. Đọc trạng thái Sprint hiện tại tại `plan/troly/sprints/sprint-XX/PLAN.md`.
2. Kiểm tra nhật ký hôm trước tại `plan/troly/scrum/DAILY_SCRUM.md`.
...
> 
> 🔄 **Kế Thừa & Nâng Cấp:** Dự án được tái cấu trúc, phát triển và nâng cấp từ `/etc/nixos/pkgs/assistant/`. Quy trình Daily Kickoff đảm bảo mọi thành phần cốt lõi của dự án cũ (Menu, Cửa sổ thiết lập, EyeLeo, WakaTracker) được rà soát và chuyển dịch tuần tự sang kiến trúc C++20/Qt6 Native theo [docs/troly/ARCHITECTURE.md](file:///etc/nixos/docs/troly/ARCHITECTURE.md).

# Quy Trình Tự Động 4 Bước Khi Kích Hoạt

### Bước 1: Quét Bối Cảnh (Context Scan)
- Đọc [plan/troly/README.md](file:///etc/nixos/plan/troly/README.md) để xác định Sprint đang hoạt động (ví dụ: `sprint-01`).
- Đọc file `PLAN.md` của Sprint đó để lấy danh sách tasks, người nhận (@DevOptAgent / @RdAgent / @AnimAgent) và trạng thái.
- Đọc mục mới nhất trong [plan/troly/scrum/DAILY_SCRUM.md](file:///etc/nixos/plan/troly/scrum/DAILY_SCRUM.md) để nắm các vướng mắc (blockers) còn tồn đọng từ hôm qua.
...
- Nếu task cần nghiên cứu kỹ thuật/trade-off: `@RdAgent` chỉ ra interface stubs hoặc POC cần làm trước theo [docs/troly/ARCHITECTURE.md](file:///etc/nixos/docs/troly/ARCHITECTURE.md).
...
- `@PlanAgent` tự động thêm một mục ngày mới vào [plan/troly/scrum/DAILY_SCRUM.md](file:///etc/nixos/plan/troly/scrum/DAILY_SCRUM.md).
