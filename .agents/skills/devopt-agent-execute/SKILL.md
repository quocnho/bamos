...
# Mục Tiêu
Viết code C++20 an toàn, hiện đại, tuân thủ Clean Architecture theo [docs/troly/ARCHITECTURE.md](file:///etc/nixos/docs/troly/ARCHITECTURE.md), đóng gói chuẩn NixOS derivation (package con của flake `/etc/nixos`) và thực hiện quy chuẩn Git Workflow tại [docs/troly/GIT_WORKFLOW.md](file:///etc/nixos/docs/troly/GIT_WORKFLOW.md).
...
- **Tiết kiệm AI token tối đa:** Đảm bảo dễ tìm kiếm thư mục, tập tin và nội dung nhỏ nhất khi đọc để tiết kiệm AI token trong quá trình dev (tham khảo [docs/troly/ANTIGRAVITY_SETUP.md](file:///etc/nixos/docs/troly/ANTIGRAVITY_SETUP.md)).
...
# Quy Chuẩn Đường Dẫn Tương Đối & Độc Lập Dự Án Con (Standalone Portability)
- **Thiết lập đường dẫn tương đối:** Vì các dự án con (`troly`, `assistant`,...) có thể chạy độc lập hoặc tách thành repo riêng, mọi đường dẫn trong mã nguồn (C++, QML), scripts, cấu hình và docs **bắt buộc phải là đường dẫn tương đối**.
- **Tuyệt đối không hardcode đường dẫn hệ thống:** Không dùng đường dẫn tuyệt đối như `/etc/nixos/pkgs/...` trong mã C++ hay QML.
- **Thực thi trong C++/Qt:** Sử dụng `QCoreApplication::applicationDirPath()`, Qt Resource System (`qrc:/`), hoặc đường dẫn tương đối từ working directory.
...
Tuân thủ đầy đủ chuẩn tại [docs/troly/GIT_WORKFLOW.md](file:///etc/nixos/docs/troly/GIT_WORKFLOW.md):
```text
