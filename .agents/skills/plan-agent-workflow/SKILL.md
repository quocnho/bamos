...
# Mục Tiêu
Quản lý luồng công việc Agile Scrum toàn diện cho dự án `troly` thông qua thư mục [plan/troly/](file:///etc/nixos/plan/troly) và đồng bộ hệ thống tài liệu kỹ thuật [docs/](file:///etc/nixos/docs/troly).
...
# Cây Thư Mục Agile Scrum Chuẩn Trong `/plan/troly/`

```text
plan/
├── README.md                   # Trung tâm quản lý lộ trình & danh mục dự án toàn hệ thống
└── troly/                      # Hồ sơ Agile Scrum của package troly
...
### 1. `plan/troly/backlog/BACKLOG.md`
Quản lý danh sách tính năng nghiệp vụ cấp cao (F1 -> F7) và phân rã các User Stories / Tasks.

### 2. `plan/troly/sprints/sprint-XX/PLAN.md` (Sprint Planning & Tracker)
```markdown
...
### 3. `plan/troly/sprints/sprint-XX/REVIEW.md` (Sprint Review)
...
### 4. `plan/troly/sprints/sprint-XX/RETRO.md` (Sprint Retrospective)
...
### 5. `plan/troly/scrum/DAILY_SCRUM.md` (Daily Scrum Tracker)
...
# Quy Trình Điều Phối Tài Liệu `/docs/troly/` & Cấu Trúc File Siêu Nhỏ (Atomic Granularity)
...
- **Đồng bộ hệ thống tài liệu chuẩn [docs/troly/](file:///etc/nixos/docs/troly):**
  - Khi có thay đổi về kiến trúc, lớp Usecases, Infrastructure, DDL hoặc Mascot: Đồng bộ ngay lập tức vào [docs/troly/ARCHITECTURE.md](file:///etc/nixos/docs/troly/ARCHITECTURE.md).
  - Khi có cập nhật về tiêu chuẩn release, git branch hoặc semantic versioning: Đồng bộ vào [docs/troly/GIT_WORKFLOW.md](file:///etc/nixos/docs/troly/GIT_WORKFLOW.md).
  - Khi có cập nhật về quy tắc Rules, Skills hoặc tối ưu Context Window: Đồng bộ vào [docs/troly/ANTIGRAVITY_SETUP.md](file:///etc/nixos/docs/troly/ANTIGRAVITY_SETUP.md).
  - Duy trì và cập nhật mục lục điều hướng tại [docs/troly/README.md](file:///etc/nixos/docs/troly/README.md) và [docs/README.md](file:///etc/nixos/docs/README.md).
