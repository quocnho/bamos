...
let
  ...
  aiServerScript = pkgs.writeShellScriptBin "bamos-ai-server" ''
    ...
    MODEL_PATH="''${BAM_AI_MODEL_PATH:-${cfg.modelPath}}"
    PORT="''${BAM_AI_PORT:-${toString cfg.port}}"
    HOST="${cfg.host}"
    NGL="''${BAM_AI_NGL:-${toString cfg.gpuLayers}}"
    CTX="''${BAM_AI_CTX:-${toString cfg.contextSize}}"
    ...
    if [ ! -f "$MODEL_PATH" ]; then
      echo -e "\033[33m[!] Model GGUF không tồn tại tại: $MODEL_PATH\033[0m"
      echo -e "\033[36m==> Bạn có thể tải tự động model Qwen2.5-1.5B bằng lệnh:\033[0m"
      echo "    bam ai pull"
      exit 1
    fi

    echo -e "\033[32m[OK]\033[0m Khởi động Local AI Server (llama-server)..."
