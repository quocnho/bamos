...
func NewFSTool() *FSTool {
	...
	return &FSTool{homeDir: home}
}

// FindFiles quét nhanh tìm tệp tin theo từ khóa hoặc đuôi mở rộng
func (ft *FSTool) FindFiles(query string, maxResults int) []string {
	if maxResults <= 0 {
		maxResults = 10
	}
	...
	// Các thư mục bỏ qua để tìm kiếm tức thì
...
func (ft *FSTool) ReadDocument(path string) (string, error) {
	// Mở rộng dấu ngã ~ thành home directory
	...
	if info.IsDir() {
		// Nếu là thư mục, liệt kê các file bên trong
	...
	// Đọc tệp tin với giới hạn 64KB để không làm tràn context SLM
