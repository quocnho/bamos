...

{
  ...
  # ==== Tag generation: tự gắn "NixOS-YY.MM.DD-HH:MM" khi switch/boot (MỌI máy) ====
  # `bam switch` / `bam boot` set env NIXOS_TAG + chạy nixos-rebuild --impure.
  # Nếu chạy `nixos-rebuild` thủ công (pure) → tryEval thất bại → không có tag,
  # không lỗi. LƯU Ý: nixpkgs chỉ cho ký tự [a-zA-Z0-9:_.-] trong tag nên
  # mẫu gốc "NixOS (YY/mm/dd-hh:mm)" viết lại: "/" → ".", "( )" → "-".
  system.nixos.tags =
    let
